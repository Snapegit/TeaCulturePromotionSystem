...
<script setup>
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			form.value.chaweishu = 0
			disabledForm.value.chaweishu = false;
		...
		context?.$http({
			...
			if(json.hasOwnProperty('yuyueshijian') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
				form.value.yuyueshijian = json.yuyueshijian
				disabledForm.value.yuyueshijian = true;
			}
	...
	const save=()=>{
		...
		if(objcross!='') {
			objcross.chaweishu = objcross.chaweishu - form.value.chaweishu
			if(objcross.chaweishu<0){
				context?.$toolUtil.message('茶位数不足','error')
				return false
			}
		}
		let crossUserId = ''
