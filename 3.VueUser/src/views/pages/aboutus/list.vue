	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"#fff","width":"80%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('aboutus','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
					...
					<mySwiper :data="list" :type="3"
						...
						<template #default="scope">
							<div class="data_item animation_box" @click.stop="detailClick(scope.row.id)" >
								<div class="data_img_box" v-if="scope.row.picture1 && scope.row.picture1.substr(0,4)=='http'" @click.stop="preViewClick(scope.row.picture1)">
									<el-image class="data_img" :src="scope.row.picture1" fit="cover"></el-image>
								</div>
								<div class="data_img_box" v-else @click.stop="preViewClick($config.url+scope.row.picture1.split(',')[0])">
									<el-image class="data_img" :src="scope.row.picture1?$config.url + scope.row.picture1.split(',')[0]:''"
										fit="cover"></el-image>
								</div>
								<div class="data_content">
									<div class="data_title">{{scope.row.title}}</div>
...
<script setup>
	...
	const detailClick = (id) => {
		router.push('aboutusDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
